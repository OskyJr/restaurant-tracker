...
export const RestaurantsView = (props: RestaurantViewProps): JSX.Element => {
  ...
  const [editRestaurant, setEditRestaurant] = useState<Omit<
    RestaurantCardProps,
    'onClickCallback'
  > | null>(null);
  ...
  return (
    ...
    >
      {data.map((el: RestaurantCardProps) => {
        return <RestaurantCard key={el.id} {...el} onClickCallback={onViewMoreCallback} />;
      })}
      {editRestaurant ? (
        <ViewEditRestaurantModal {...editRestaurant} onClickCallback={onCloseCallback} />
      ) : null}
    </Box>
  );
};
